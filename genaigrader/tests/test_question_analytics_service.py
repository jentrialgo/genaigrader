...
from genaigrader.models import Course, Evaluation, Exam, Model, Question, QuestionEvaluation, QuestionOption
...
class QuestionAnalyticsServiceTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(username='analytics_user', password='secret')
        self.course = Course.objects.create(name='Course', user=self.user)
        self.exam = Exam.objects.create(course=self.course, description='Exam', user=self.user)

        self.question = Question.objects.create(statement='What is correct?', exam=self.exam)
        self.option_a = QuestionOption.objects.create(question=self.question, content='a) Correct')
        self.option_b = QuestionOption.objects.create(question=self.question, content='b) Wrong')
