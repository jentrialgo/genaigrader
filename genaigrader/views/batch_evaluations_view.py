...
def generate_eval_tasks(
    exams: Iterable, models: Iterable, repetitions: int
) -> Generator:
    """
        ...
        try:
            ...
            if {"correct_count", "total_time", "total_questions"}.issubset(data):
                ...
                normalized_score = round(
                    data["correct_count"] / data["total_questions"] * 10, 2
                )
                return {
...
def batch_stream(
    exams_to_eval: Iterable,
    models_to_eval: Iterable,
    repetitions: int,
    user_prompt: str,
    notes: str = None,
    ...
    for exam, model, rep in generate_eval_tasks(
        exams_to_eval, models_to_eval, repetitions
    ):
        try:
        ...
        try:
            ...
            for chunk in stream_responses(
                questions, user_prompt, llm, len(questions), exam, notes
            ):
...
def handle_batch_evaluations_post(request, user, exams, models):
    ...
    logging.warning(
        f"selected_exam_ids: {selected_exam_ids}, selected_model_ids: {selected_model_ids}, repetitions: {repetitions}, user_prompt: {user_prompt}, notes: {notes}"
    )

    exams_to_eval = exams.filter(id__in=selected_exam_ids)
    models_to_eval = [m for m in models if str(m.id) in selected_model_ids]
    logging.warning(
        f"exams_to_eval: {[e.id for e in exams_to_eval]}, models_to_eval: {[m.id for m in models_to_eval]}"
    )
